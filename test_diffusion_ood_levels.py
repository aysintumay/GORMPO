...
# Add parent directory and diffusion directory to path for imports
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
diffusion_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'diffusion')
...
def evaluate_ood_at_distance(model, dataset_name, distance, ood_test_dir='/public/d4rl/ood_test', device='cpu', mean=None, std=None):
    ...
    for i in range(0, len(test_data_tensor), batch_size):
        ...
        batch_log_probs = model.score_samples(batch, batch_size=batch_size)
...
def main():
    parser = argparse.ArgumentParser(description='Test Diffusion model on different OOD distance levels')
    parser.add_argument('--model_path', type=str, required=True,
                       help='Path to saved Diffusion model checkpoint (e.g., /public/gormpo/models/hopper/diffusion/model.pt)')
    parser.add_argument('--scheduler_dir', type=str, required=True,
                       help='Path to scheduler config directory (e.g., /public/gormpo/models/hopper/diffusion/scheduler)')
    ...
    parser.add_argument('--save_dir', type=str, default='figures/diffusion_ood_distance_tests',
                       help='Directory to save results')
    parser.add_argument('--num_inference_steps', type=int, default=20,
                       help='Number of timesteps for ELBO computation (default: 20, use None for all)')
    ...
    # Load model
    print(f"\nLoading Diffusion model from: {args.model_path}")
    model, cfg = build_model_from_ckpt(args.model_path, device)

    # Load scheduler
    print(f"Loading scheduler from: {args.scheduler_dir}")
    try:
        scheduler = DDIMScheduler.from_pretrained(args.scheduler_dir)
        print("Loaded DDIM scheduler")
    except Exception:
        try:
            scheduler = DDPMScheduler.from_pretrained(args.scheduler_dir)
            print("Loaded DDPM scheduler")
        except Exception as e:
            print(f"Warning: Could not load scheduler from {args.scheduler_dir}: {e}")
            print("Using default DDIM scheduler")
    ...
    # Wrap in OOD detector
    ood_model = DiffusionOOD(model, scheduler, device=device, num_inference_steps=args.num_inference_steps)

    # Load metadata if available
    metadata_path = args.model_path.replace('/model.pt', '').replace('/checkpoint.pt', '')
    # Extract environment name from dataset
    env_name = args.dataset_name.split('-')[0]
    metadata_file = f"diffusion/{env_name}_ood_metadata.pkl"
    ...
    print("\n" + "="*80)
    print("Testing Diffusion Model on Different OOD Distance Levels")
    print("="*80)
