...
def main():
    ...
    save_dir = args.save_dir if args.save_dir is not None else config.get('save_dir', 'figures/vae_ood_distance_tests')
    ...
    # Load model
    print(f"\nLoading VAE model from: {model_path}")
    model_dict = VAE.load_model(model_path)
    ...
    plot_results(all_results, save_dir=final_save_dir, model_name='VAE', threshold=model.threshold)
