...
def load_ood_test_data(dataset_name, distance, base_path='/public/d4rl/ood_test', suffix=''):
    ...
    Args:
        ...
        distance: OOD distance level (int or float, e.g., 0.5, 0.8, 1, 2, 3, 4)
    ...
    """
    # Format distance value - use int if it's a whole number, otherwise keep as float
...
def evaluate_ood_at_distance(model, dataset_name, distance, base_path='/public/d4rl/ood_test', device='cpu', suffix=''):
    """
...
def main():
    ...
    save_dir = args.save_dir if args.save_dir is not None else config.get('save_dir', 'figures/kde_ood_distance_tests')
    ...
    # Load model
    print(f"\nLoading KDE model from: {model_path}")
    use_gpu = device.startswith('cuda')
    devid = int(device.split(':')[1]) if ':' in device else 0

    model_dict = PercentileThresholdKDE.load_model(model_path, use_gpu=use_gpu, devid=devid)
    ...
    plot_results(all_results, save_dir=final_save_dir, model_name='KDE', threshold=model.threshold)
