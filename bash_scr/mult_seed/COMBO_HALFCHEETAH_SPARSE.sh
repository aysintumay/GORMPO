#!/bin/bash
set -e
# COMBO on HalfCheetah-Medium-Expert (sparse D4RL), 3 seeds.
...
#
# Identical to COMBO_HALFCHEETAH.sh except it trains on the sparse dataset via
# run_combo_in_sparse.py --dataset-path (instead of the env's default D4RL data).
#
# Usage (from GORMPO root):
#   conda activate LEQ2
#   CUDA_VISIBLE_DEVICES=2 bash bash_scr/mult_seed/COMBO_HALFCHEETAH_SPARSE.sh

TASK="${TASK:-halfcheetah-medium-expert-v2}"
DATASET_PATH="${DATASET_PATH:-/public/d4rl/sparse_datasets/halfcheetah_medium_expert_sparse_72.5.pkl}"
