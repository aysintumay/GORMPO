#!/bin/bash
set -e
# COMBO on Hopper-Medium-Expert (sparse D4RL), 3 seeds.
...
#
# Identical to COMBO_HOPPER.sh except it trains on the sparse dataset via
# run_combo_in_sparse.py --dataset-path (instead of the env's default D4RL data).
#
# Usage (from GORMPO root):
#   conda activate LEQ2
#   CUDA_VISIBLE_DEVICES=2 bash bash_scr/mult_seed/COMBO_HOPPER_SPARSE.sh
