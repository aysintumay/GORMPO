#!/bin/bash
# Threshold sensitivity sweep: GORMPO-Diffusion, HalfCheetah-Medium-Expert-v2 (Sparse 72.5%)
# Usage: bash bash_scr/mult_thr/gormpo_diffusion_halfcheetah_medium_expert_sparse3.sh
...
RESULTS_FILE="results/thr_experiments/diffusion/halfcheetah_medium_expert_sparse3_individual.csv"
AGG_FILE="results/thr_experiments/diffusion/halfcheetah_medium_expert_sparse3_aggregated.csv"
...
for seed in "${seeds[@]}"; do
    ...
    # Step 1: compute & save threshold_candidates into checkpoint_metadata.pkl
    python diffusion/ddim_training_unconditional.py \
        ...
        --out /public/gormpo/models/halfcheetah_medium_expert_sparse_3_next/diffusion \
    ...
    for thr in "${thresholds[@]}"; do
        ...
        # Step 2: train GORMPO-Diffusion policy using the pre-computed threshold
        python mopo.py \
            --config configs/diffusion/gormpo_halfcheetah_medium_expert_sparse_3.yaml \
            --seed $seed \
            --classifier_model_name /public/gormpo/models/halfcheetah_medium_expert_sparse_3_next/diffusion/checkpoint.pt \
