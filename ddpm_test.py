...
def ddpm_stochastic_sample(
    ...
    """
    DDPM stochastic sampling for cond→target.
    This is the standard DDPM sampling procedure that adds noise at each step.
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate DDPM inference on test NPZ",
        parents=[config_only],
    )
    ...
    # Note: We need to create a DDPMScheduler, but the saved scheduler from training might be DDIMScheduler
    # We'll try to load it, but if it fails, we'll create a new DDPMScheduler with the same config
    model, train_cfg = build_model_from_ckpt(ckpt_path, device)
    
    try:
        # Try to load as DDPMScheduler first
        scheduler = DDPMScheduler.from_pretrained(sched_dir)
    except Exception:
        # If that fails, try loading as DDIMScheduler and convert
        try:
            from diffusers.schedulers.scheduling_ddim import DDIMScheduler
            ddim_scheduler = DDIMScheduler.from_pretrained(sched_dir)
            # Create a DDPMScheduler with the same parameters
            scheduler = DDPMScheduler(
            ...
            # Fallback: create a default DDPMScheduler
            print(f"Warning: Could not load scheduler from {sched_dir}: {e}")
            print("Creating default DDPMScheduler with 1000 timesteps")
            scheduler = DDPMScheduler(
